...
static HMODULE g_hClientDLL;
static DWORD g_dwClientDLLSize;

static HMODULE g_hEngineDLL;
static DWORD g_dwEngineDLLSize;

typedef BOOL( WINAPI *fnGetCursorPos )(LPPOINT lpPoint);
typedef BOOL( WINAPI *fnSetCursorPos )(int X, int Y);


static fnGetCursorPos g_pfnGetCursorPos;
static fnSetCursorPos g_pfnSetCursorPos;

static HWND g_hWnd;
static RAWINPUTDEVICE g_rawDevice;
static ULONG g_uButtons;
static POINT g_ptCursorPos;
...
void MF_Initialize( void )
{
	...
	WndClassEx.lpszClassName = TEXT( "CSMouseFix" );
